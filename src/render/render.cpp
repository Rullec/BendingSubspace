...
// #include "kinect/RenderResource.h"
...
void cRender::MouseMoveCallback(double xpos, double ypos)
{
    ...
    //     int cur_width = cur_resource->mWidth,
    //         cur_height = cur_resource->mHeight;
    ...
    //     float r = cur_resource->mPixelData[3 * idx + 0];
    //     float g = cur_resource->mPixelData[3 * idx + 1];
    //     float b = cur_resource->mPixelData[3 * idx + 2];
    //     if (std::fabs(r - g) < 1e-6 && std::fabs(b - g) < 1e-6)
    //     {
    //         printf("\r[info] cur pixel depth value %d mm", int(r * 1e3 ));
    //     }
    //     else
    //     {
    //         printf("\r[info] cur pixel RGB value %.2f, %.2f, %.2f", r, g, b);
    //     }
    // }
}
...
void cRender::SetRenderablePtr(cRenderablePtr mana)
{
    mRenderInterior = mana;
}
bool mNeedToUpdateImGuiWindowPos = true;
...
void cRender::UpdateTextureFromRenderResource(cRenderResourcePtr resource)
{
    ...
    memcpy(mTextureData.data(), resource->mPixelData.data(), sizeof(float) * (new_height * new_width * resource->mChannels));
    std::cout << "update texture from rendering resource!\n";
}

void cRender::UpdateTextureFromRenderResourceVec(std::vector<cRenderResourcePtr> resource)
{
    ...
    while (i < resource.size())
    {
        ...
        image_shape_array.push_back(
            tVector2i(cur_res->mHeight, cur_res->mWidth));
        // printf("[debug] render resource %d height %d width %d\n", i, cur_res->mHeight, cur_res->mWidth);
        rendering_resouce_array.push_back(cur_res->mPixelData.data());

        tex_height_row1 = std::max(cur_res->mHeight, tex_height_row1);
        tex_width_row1 += cur_res->mWidth;
        i++;
    }
